...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            uploadButtonClass: "ajax-file-upload blue_button",
        ...
        var uploadLabel = $('<div>' + s.uploadButtonName + $(this).html() + '</div>');
        ...
        if (s.loadedFiles) {
            for (var i in s.loadedFiles) {
                var pd = new createProgressDiv(obj, s);
                pd.filename.html(s.loadedFiles[i][1]);
                pd.del.click(function (i, pd) {
                    return function () {
                        if (s.deleteCallback) s.deleteCallback.call(this, {success: [[s.loadedFiles[i][0], s.loadedFiles[i][1], s.loadedFiles[i][2]]]}, pd);
                    }
                }(i, pd));
                pd.del.show();
            }
        ...
        function createCutomInputFile(obj, group, s, uploadLabel) {
			...
			uploadLabel.css({position: 'relative',overflow:'hidden',cursor:'pointer',margin:'0'});
